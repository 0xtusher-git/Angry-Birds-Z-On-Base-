...
const Engine = {
  ...
  loadLevel(levelDef) {
    ...
    // Ground baseline
    const groundY = H * 0.88;
    // For portrait, we might need to shift structures up to use vertical space
    // or scale them to fit the narrow width.
    ...
    for (const bd of levelDef.blocks) {
      ...
      const bx = bd.x * W;
      const by = isPortrait ? (bd.y * H - H * 0.35) : (bd.y * H);
    ...
    for (const pd of levelDef.pigs) {
      ...
      const px = pd.x * W;
      const py = isPortrait ? (pd.y * H - H * 0.35) : (pd.y * H);
